...
import { Plus, Search, Eye, Edit2, Trash2, Mail, Phone } from 'lucide-react';
import Topbar from '../layout/Topbar';
import { generateId } from '../../data/store';

const emptyTeacher = {
  firstName: '', lastName: '', dob: '', gender: '', qualification: '',
  subjects: [], hireDate: '', status: 'Active', phone: '', email: '', nationalId: ''
};

const ALL_SUBJECTS = [
  'Mathematics', 'Further Mathematics', 'English Language', 'English Literature',
  'Physics', 'Chemistry', 'Biology', 'Science', 'History', 'Geography',
  'Computer Science', 'ICT', 'Art', 'Music', 'Physical Education', 'Business Studies'
];

function TeacherModal({ mode, teacher, onSave, onClose }) {
  const [form, setForm] = useState(teacher ? { ...teacher, subjects: teacher.subjects || [] } : emptyTeacher);
    ...
    <div className="modal-overlay">
      <div className="modal modal-lg">
        <div className="modal-header">
          <div className="modal-title">{mode === 'add' ? 'Add New Teacher' : 'Edit Teacher'}</div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="modal-body">
            ...
            <div className="form-group">
              ...
              <input className="form-control" value={form.qualification} onChange={e => set('qualification', e.target.value)} placeholder="e.g. BSc Mathematics, PGCE" />
            </div>
            <div className="form-row">
              <div className="form-group">
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input type="date" className="form-control" value={form.hireDate} onChange={e => set('hireDate', e.target.value)} />
              </div>
              <div className="form-group">
...
export default function Teachers({ store, onUpdate }) {
  ...
  const filtered = useMemo(() =>
    store.teachers.filter(t => {
      ...
      return !q || t.firstName.toLowerCase().includes(q) || t.lastName.toLowerCase().includes(q) ||
  ...
  function handleSave(data) {
    const updated = { ...store };
    if (modal === 'add') {
      updated.teachers = [...store.teachers, { ...data, id: generateId('T') }];
    } else {
  ...
  function handleDelete(id) {
    ...
    onUpdate({ ...store, teachers: store.teachers.filter(t => t.id !== id) });
  }
  ...
  return (
    <div>
      <Topbar
        title="Teachers"
        subtitle={`${store.teachers.length} staff members`}
        school={store.school}
        actions={
      ...
      <div className="page-content animate-in">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
          {filtered.map((teacher, i) => {
            ...
            return (
              <div key={teacher.id} className="card" style={{ transition: 'all 0.2s' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 14 }}>
                  <div className={`avatar avatar-${AVATARS[i % 4]}`} style={{ width: 48, height: 48, fontSize: 16 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ marginTop: 6 }}>
                      <span className={`badge ${teacher.status === 'Active' ? 'badge-success' : 'badge-warning'}`}>{teacher.status}</span>
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: 4 }}>
                    <button className="btn btn-ghost btn-sm btn-icon" onClick={() => { setSelected(teacher); setModal('edit'); }}>
                      <Edit2 size={13} />
                    </button>
                    <button className="btn btn-danger btn-sm btn-icon" onClick={() => handleDelete(teacher.id)}>
                      <Trash2 size={13} />
                    </button>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 12 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: 'var(--text3)' }}>
                    <Mail size={12} /> {teacher.email || 'No email'}
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: 'var(--text3)' }}>
                    <Phone size={12} /> {teacher.phone || 'No phone'}
                  </div>
                </div>
              </div>
            );
